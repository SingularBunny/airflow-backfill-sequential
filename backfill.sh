...
AIRFLOW_POD=$(kubectl get pods -n airflow | grep -e web -e Running | head -n1 | awk '{print $1;}')
...
for dt in "${DATES[@]}"; do
  ...
  kubectl exec -it -n airflow "$AIRFLOW_POD" -- airflow dags backfill -s "$dt" -e "$(date -d "${dt} + 1 day" "+%Y-%m-%d")" "$(if [ $RUN_BACKWARDS ]; then echo "-B"; fi)" "${POSITIONAL[@]}"
done